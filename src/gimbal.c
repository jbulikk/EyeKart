...
static const limits_t yaw_limits = {
    .min_angle = -45.0f,
    .max_angle = 45.0f
};

static const limits_t pitch_limits = {
    .min_angle = -15.0f,
    .max_angle = 15.0f
};
...
int gimbal_set_yaw(float angle_deg) {
    if (angle_deg < yaw_limits.min_angle) angle_deg = yaw_limits.min_angle;
    if (angle_deg > yaw_limits.max_angle) angle_deg = yaw_limits.max_angle;
    return angle_servo_set_angle(&gimbal_yaw, angle_deg);
}

int gimbal_set_pitch(float angle_deg) {
    if (angle_deg < pitch_limits.min_angle) angle_deg = pitch_limits.min_angle;
    if (angle_deg > pitch_limits.max_angle) angle_deg = pitch_limits.max_angle;
